...
open class RippleButton: UIButton {
    ...
    @IBInspectable open var rippleBackgroundColor: UIColor = UIColor(white: 1, alpha: 0) {
        didSet {
    ...
    @IBInspectable open var touchUpAnimationTime: Double = 1
...
//        let size: CGFloat = bounds.size.width * CGFloat(ripplePercent)
        let size: CGFloat = bounds.width
        let x: CGFloat = (bounds.width/2) - (size/2)
        let y: CGFloat = (bounds.height/2) - (size/2)
        let corner: CGFloat = size/2
        
        rippleView.backgroundColor = rippleColor
        rippleView.frame = CGRect(x: x, y: y, width: size * 2, height: size * 2)
        rippleView.layer.cornerRadius = corner * 2
    }
